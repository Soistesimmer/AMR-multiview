...
class NMTModel(nn.Module):
    ...
    def forward(self, src, tgt, stgt, structure, mask, lengths):
        ...
        self.ddecoder.init_state(tgt, dec_out)
        ddec_out, d_attns = self.ddecoder(stgt[:-1])
        if mask is not None:
            bi_out, label_out = self.biaffine(dec_out[1:], mask)
            return dec_out, attns, ddec_out, d_attns, bi_out, label_out
        return dec_out, attns, ddec_out, d_attns
...
def build_embeddings(opt, word_dict, for_encoder='src'):
    ...
    if for_encoder == 'src' or for_encoder == 'tgt' or for_encoder == 'stgt':
...
def build_base_model(model_opt, fields, gpu, checkpoint=None):
    ...
    # Build decoder.
    decoder = [build_decoder(model_opt, tgt_embeddings), build_decoder(model_opt, src_embeddings)]
    ...
    # Build Generator. src and tgt share the same vocab, no need another one for second decoder
    gen_func = nn.LogSoftmax(dim=-1)
    ...
    if model_opt.share_decoder_embeddings:
        generator[0].weight = decoder[0].embeddings.word_lut.weight
