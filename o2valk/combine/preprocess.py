...
def build_vocab(train_dataset_files, fields, share_vocab,
                ...
                structure_vocab_size, structure_words_min_frequency, relation_vocab_size):
    counter = {}
    ...
    build_field_vocab(fields["stgt"], counter["stgt"],
                      max_size=src_vocab_size,
                      min_freq=src_words_min_frequency)
    logger.info(" * stgt vocab size: %d." % len(fields["stgt"].vocab))
    ...
    if share_vocab:
        ...
        merged_vocab = merge_vocabs(
            [fields["src"].vocab, fields["tgt"].vocab, fields["stgt"].vocab],
            vocab_size=src_vocab_size,
            min_frequency=src_words_min_frequency)
        fields["src"].vocab = merged_vocab
        fields["tgt"].vocab = merged_vocab
        fields["stgt"].vocab = merged_vocab
        logger.info(" * src vocab size: %d." % len(fields["src"].vocab))
        logger.info(" * tgt vocab size: %d." % len(fields["tgt"].vocab))
        logger.info(" * stgt vocab size: %d." % len(fields["stgt"].vocab))
...
def build_save_in_shards_using_shards_size(src_corpus, tgt_corpus, stgt_corpus, structure_corpus, mask_corpus, relation_corpus, fields, corpus_type,
                                           opt):
    src_data = []
    tgt_data = []
    stgt_data = []
    ...
    with open(src_corpus, "r") as src_file:
        with open(tgt_corpus, "r") as tgt_file:
            with open(structure_corpus, "r") as structure_file:
                with open(stgt_corpus, 'r') as stgt_file:
                    with open(mask_corpus, 'r') as mask_file:
                        with open(relation_corpus, 'r')as relation_file:
                            for s, t, st, structure, mask, relation in zip(src_file, tgt_file, stgt_file, structure_file, mask_file, relation_file):
                                src_data.append(s)
                                tgt_data.append(t)
    ...
    for x in range(num_shards):
        ...
        f = codecs.open(stgt_corpus + ".{0}.txt".format(x), "w", encoding="utf-8")
        f.writelines(stgt_data[x * opt.shard_size: (x + 1) * opt.shard_size])
        f.close()
        ...
        f = codecs.open(relation_corpus + ".{0}.txt".format(x), "w", encoding="utf-8")
    ...
    if len(src_data) > num_written:  # 处理最后一个剩下的shard
        ...
        f = codecs.open(stgt_corpus + ".{0}.txt".format(num_shards), 'w', encoding="utf-8")
        f.writelines(stgt_data[num_shards * opt.shard_size:])
        f.close()
        ...
        f = codecs.open(relation_corpus + ".{0}.txt".format(num_shards), "w", encoding="utf-8")
    ...
    stgt_list = sorted(glob.glob(stgt_corpus + '.*.txt'))
    ...
    for index, src in enumerate(src_list):
        ...
        stgt_iter = make_text_iterator_from_file(stgt_list[index])
...
def build_save_vocab(train_dataset, fields, opt):
    ...
    fields = build_vocab(train_dataset,
                         ...
                         opt.structure_words_min_frequency, opt.relation_vocab_size)
    ...
    store_vocab_to_file(fields['stgt'].vocab, opt.save_data + '_stgt_vocab')
...
def build_save_dataset(corpus_type, fields, opt):  # corpus_type: train or valid
    ...
    if (opt.shard_size > 0):
        return build_save_in_shards_using_shards_size(src_corpus, tgt_corpus, stgt_corpus, structure_corpus, mask_corpus, relation_corpus, fields, corpus_type,
                                                      opt)
