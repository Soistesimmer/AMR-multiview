...
def make_features(batch, side):
    ...
    """
    assert side in ['src', 'tgt', 'structure', 'mask']
...
def load_fields(opt, checkpoint):
    ...
    logger.info(' * vocabulary size. source = %d; target = %d; structure = %d; mask = %d;' %
                (len(fields['src'].vocab), len(fields['tgt'].vocab), len(fields['structure'].nesting_field.vocab), len(fields['mask'].vocab)))
...
def build_dataset(fields,
    ...
    dataset = Dataset(fields, src_examples_iter, tgt_examples_iter, structure_examples_iter,mask_examples_iter,
...
class Dataset(torchtext.data.Dataset):
    def __init__(self, fields, src_examples_iter, tgt_examples_iter, structure_examples_iter, mask_examples_iter,
                 src_seq_length=0, tgt_seq_length=0,
                 use_filter_pred=True):
        ...
        if tgt_examples_iter is not None and structure_examples_iter is not None and mask_examples_iter is not None:
            examples_iter = (_join_dicts(src, tgt, structure, mask) for src, tgt, structure, mask in
                             zip(src_examples_iter, tgt_examples_iter, structure_examples_iter, mask_examples_iter))
