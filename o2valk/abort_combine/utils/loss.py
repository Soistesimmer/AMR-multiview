...
class LossComputeBase(nn.Module):
    ...
    def sharded_compute_loss(self, batch, output, attns, shard_size,
        ...
        shard_state = self._make_shard_state(batch, output, attns)
        for shard in shards({k: v[0] for k, v in shard_state.items()}, shard_size, retain_graph=True):
            loss, stats = self._compute_loss(batch, **shard)
            loss *= ratio1
            loss.div(float(normalization)).backward(retain_graph=True)
            batch_stats.update(stats)
        for shard in shards({k: v[1] for k, v in shard_state.items()}, shard_size, retain_graph=True):
            loss, stats = self._compute_loss(batch, **shard)
            loss *= ratio2
            loss.div(float(normalization)).backward(retain_graph=True)
            batch_stats.update(stats)
