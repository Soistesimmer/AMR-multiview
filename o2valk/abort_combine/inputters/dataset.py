...
def make_features(batch, side):
    ...
    """
    assert side in ['src', 'tgt', 'structure', 'mask', 'stgt']
...
def get_source_fields(fields=None):
    ...
    # eos_token append EOS to the text
...
def get_starget_fields(fields=None):
    if fields is None:
        fields = {}
    ...
    fields["stgt"] = torchtext.data.NestedField(nesting_field,pad_token=Constants.PAD_WORD)
...
def get_mask_fields(fields=None):
    if fields is None:
        fields={}
...
def get_fields():
    ...
    fields = get_starget_fields(fields)
    fields = get_mask_fields(fields)
...
def build_dataset(fields,
    ...
    if stgt_data_iter != None:
        stgt_examples_iter = Dataset.make_stgt_nested_examples(stgt_data_iter, abundancy, src_seq_length_trunc, "stgt")
    else:
        stgt_examples_iter = None
    ...
    dataset = Dataset(fields, src_examples_iter, tgt_examples_iter, stgt_examples_iter, structure_examples_iter,mask_examples_iter,
...
class Dataset(torchtext.data.Dataset):
    def __init__(self, fields, src_examples_iter, tgt_examples_iter, stgt_examples_iter, structure_examples_iter, mask_examples_iter,
                 src_seq_length=0, tgt_seq_length=0,
                 use_filter_pred=True):
        ...
        if tgt_examples_iter is not None and structure_examples_iter is not None and mask_examples_iter is not None:
            examples_iter = (_join_dicts(src, tgt, stgt, structure, mask) for src, tgt, stgt, structure, mask in
                             zip(src_examples_iter, tgt_examples_iter, stgt_examples_iter, structure_examples_iter, mask_examples_iter))
            out_fields = get_target_fields(out_fields)
            out_fields=get_starget_fields(out_fields)
    ...
    @staticmethod
    def make_stgt_nested_examples(text_iter, abundancy, truncate, side):
        for i, line in enumerate(text_iter):
            line = line.strip().split()
            length = len(line)
            src_length = int(length/abundancy)
            words = (line[j: j + src_length] for j in range(0, len(line), src_length))
            example_dict = {side: tuple(words), "indices": i}
            yield example_dict
