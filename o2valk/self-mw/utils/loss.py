...
class NMTLossCompute(LossComputeBase):
  ...
  def _make_shard_state(self, batch, output):
    return {
        "output": output,
        "target": (batch.tgt,batch.src),
    }

  def _compute_loss(self, batch, output, target):
    f_output=output[0]
    s_output=output[1]
    f_bottled_output = self._bottle(f_output)
    s_bottled_output = self._bottle(s_output)
    f_scores = self.generator(f_bottled_output)
    s_scores = self.generator(s_bottled_output)

    f_target=target[0]
    s_target=target[1]
    f_gtruth = f_target.view(-1)
    s_gtruth=s_target.view(-1)

    loss = self.criterion(f_scores, f_gtruth)+self.criterion(s_scores, s_gtruth)
    stats = self._stats(loss.clone(), f_scores, f_gtruth)
