...
from onmt.transformer_sub_encoder import TransformerEncoder as SubTransformerEncoder
from onmt.transformer_decoder import TransformerDecoder
from onmt.sublayer import SelfAttention
...
class NMTModel(nn.Module):
    def __init__(self, encoder, decoder, sub_encoder, variation):
        super(NMTModel, self).__init__()
        self.encoder = encoder
        self.decoder = decoder
        self.sub_encoder = sub_encoder
        self.variation = variation
        self.kll = nn.KLDivLoss(reduce=True)

    def forward(self, src, tgt, structure, lengths, t_lengths=None):
        sub_memory_bank=None
        if t_lengths is not None:
            _, sub_memory_bank, _ = self.sub_encoder(tgt, t_lengths)
        _, memory_bank, lengths = self.encoder(src, structure, lengths)  # src: ......<EOS>
        full_z, pred_z = self.variation(memory_bank, sub_memory_bank)
        self.decoder.init_state(src, memory_bank)
        tgt = tgt[:-1]  # exclude last target from inputs
        dec_out, attns = self.decoder(tgt, pred_z)
        if full_z is None:
            return dec_out, attns
        return dec_out, attns, self.kll(full_z, pred_z)


class VariG(nn.Module):
    def __init__(self, hidden_size, dropout):
        super(VariG, self).__init__()
        self.self_src_attn = SelfAttention(hidden_size, dropout)
        self.self_tgt_attn = SelfAttention(hidden_size, dropout)
        self.pre_c = nn.Sequential(nn.Linear(hidden_size * 2, hidden_size), nn.Tanh())
        self.mean = nn.Linear(hidden_size, hidden_size)
        self.diff = nn.Linear(hidden_size, hidden_size)
        self.lem_c = nn.Sequential(nn.Linear(hidden_size, hidden_size), nn.Tanh())

    def forward(self, src_input, tgt_input=None):
        print(src_input.size(),tgt_input.size())
        src_out = self.self_src_attn(src_input)
        tgt_out = self.self_tgt_attn(tgt_input)
        print(src_out.size(),tgt_out.size())
        out2 = self.lem_c(src_out)
        miu2 = self.mean(out2)
        omega2 = self.mean(out2)
        sample=torch.randn(miu2.size(0), miu2.size(1)).cuda()
        z2 = miu2 + torch.sqrt(torch.exp(omega2)).mul(sample)
        if tgt_input is not None:
            out = self.pre_c(torch.cat((src_out, tgt_out),dim=2))
            miu = self.mean(out)
            omega = self.diff(out)
            sample = torch.randn(miu2.size(0), miu2.size(1)).cuda()
            z1 = miu + torch.sqrt(torch.exp(omega)).mul(sample)
            return z1, z2
        return None, z2
...
def build_sub_encoder(opt, embeddings):
    return SubTransformerEncoder(opt.enc_layers, opt.enc_rnn_size, opt.heads, opt.transformer_ff, opt.dropout,
                                 embeddings)
...
def build_variation(opt):
    return VariG(opt.dec_rnn_size, opt.dropout)
...
def build_base_model(model_opt, fields, gpu, checkpoint=None):
    ...
    sub_encoder = build_sub_encoder(model_opt, tgt_embeddings)
    variation = build_variation(model_opt)
    # Build NMTModel(= encoder + decoder).
    device = torch.device("cuda:0" if gpu else "cpu")
    model = NMTModel(encoder, decoder, sub_encoder, variation)
