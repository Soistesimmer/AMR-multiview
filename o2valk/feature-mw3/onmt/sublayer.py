...
class SelfAttention(nn.Module):
    def __init__(self, hidden_size, dropout):
        ...
        self.ql=nn.Linear(hidden_size,1)
        self.kl=nn.Linear(hidden_size,hidden_size)
        self.vl=nn.Linear(hidden_size, hidden_size)
        self.softmax=nn.Softmax(dim=-1)
    ...
    def forward(self, input):
        key=self.kl(input)
        value=self.vl(input)
        attn=self.softmax(self.ql(key).reshape(key.size(0),-1))
        drop_attn=self.dp(attn).unsqueeze(1)
        print(drop_attn.size(), value.size())
        out=drop_attn.bmm(value).squeeze(1)
        print(out.size())
        return out
