...
def build_save_in_shards_using_shards_size(src_corpus, tgt_corpus, stgt_corpus, structure_corpus, fields, corpus_type,
                                           opt):
    src_data = []
    tgt_data = []
    stgt_data = []
        ...
        with open(tgt_corpus, "r") as tgt_file:
            with open(structure_corpus, "r") as structure_file:
                with open(stgt_corpus, 'r') as stgt_file:
                    for s, t, st, structure in zip(src_file, tgt_file, stgt_file, structure_file):
                        src_data.append(s)
                        tgt_data.append(t)
...
def build_save_dataset(corpus_type, fields, opt):  # corpus_type: train or valid
    ...
    if (opt.shard_size > 0):
        return build_save_in_shards_using_shards_size(src_corpus, tgt_corpus, stgt_corpus, structure_corpus, fields, corpus_type,
                                                      opt)
